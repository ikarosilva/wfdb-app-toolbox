function varargout = wfdbviewer(varargin)
% WFDBVIEWER MATLAB code for wfdbviewer.fig
%      WFDBVIEWER, by itself, creates a new WFDBVIEWER or raises the existing
%      singleton*.
%
%      H = WFDBVIEWER returns the handle to a new WFDBVIEWER or the handle to
%      the existing singleton*.
%
%      WFDBVIEWER('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in WFDBVIEWER.M with the given input arguments.
%
%      WFDBVIEWER('Property','Value',...) creates a new WFDBVIEWER or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before wfdbviewer_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to wfdbviewer_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help wfdbviewer

% Last Modified by GUIDE v2.5 12-Dec-2014 15:43:16

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @wfdbviewer_OpeningFcn, ...
    'gui_OutputFcn',  @wfdbviewer_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before wfdbviewer is made visible.
function wfdbviewer_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to wfdbviewer (see VARARGIN)

global current_record records tm tm_step

% Choose default command line output for wfdbviewer
...
% UIWAIT makes wfdbviewer wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = wfdbviewer_OutputFcn(~,~, handles)
...
[N,CH]=size(signal);
%Plot annotations along with data
AMP=[1 0.25 0.5 0.95 1.5 2 3];
amp = AMP(get(handles.AmplitudeScale,'Value'));
offset=0.5;
DC=min(signal);
...
TM_SC=[tm(end)-tm(1) 60*2 30*2 10*2 1*2];
